...
int main(int argc, char *argv[])
{
	...
	b_console_c =
	    GTK_TEXT_BUFFER(gtk_builder_get_object(builder, "b_console_c"));
	b_console_e =
	    GTK_TEXT_BUFFER(gtk_builder_get_object(builder, "b_console_e"));
	scroll_view_c =
	    GTK_TEXT_VIEW(gtk_builder_get_object(builder, "console_c"));
	scroll_view_e =
	    GTK_TEXT_VIEW(gtk_builder_get_object(builder, "console_e"));
...
void check_fields_c()
{
	if (strlen(gtk_entry_get_text(pass_c)) == 0
	    || strlen(gtk_entry_get_text(image_c_in)) == 0
	    || strlen(gtk_entry_get_text(image_c_out)) == 0
	    || strlen(gtk_entry_get_text(text)) == 0)
		on_error();
	else {
		char *t;
		asprintf(&t, "./hips_c %s \"%s\" %s %s 2>&1",
			 gtk_entry_get_text(pass_c), gtk_entry_get_text(text),
			 gtk_entry_get_text(image_c_in),
			 gtk_entry_get_text(image_c_out));
		g_print("%s", t);
		g_print("\n");
		execute(t, 0);
		free(t);
...
void check_fields_e()
{
	if (strlen(gtk_entry_get_text(pass_e)) == 0
	    || strlen(gtk_entry_get_text(image_e)) == 0)
		on_error();
	else {
		char *t;
		asprintf(&t, "./hips_e %s %s 2>&1", gtk_entry_get_text(pass_e),
			 gtk_entry_get_text(image_e));
		g_print("%s", t);
		g_print("\n");
		execute(t, 1);
		free(t);
...
void execute(char *com, int where)
{
	...
	while (fgets(buff, sizeof(buff), in) != NULL) {
		if (where == 0) {
			gtk_text_buffer_insert_at_cursor(b_console_c, buff,
							 strlen(buff));
			gtk_text_view_scroll_to_mark(scroll_view_c,
						     gtk_text_buffer_get_insert
						     (b_console_c), 0.0, TRUE,
						     0.5, 0.5);
		} else {
			gtk_text_buffer_insert_at_cursor(b_console_e, buff,
							 strlen(buff));
			gtk_text_view_scroll_to_mark(scroll_view_e,
						     gtk_text_buffer_get_insert
						     (b_console_e), 0.0, TRUE,
						     0.5, 0.5);
		}
	}
	if (where == 0) {
